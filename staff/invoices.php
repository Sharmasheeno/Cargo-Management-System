<?php
// staff/invoices.php
// Invoices Management for Staff (role_type: finance_manager / clerk)
// Adapted from branch_manager/invoices.php. Finance/clerk staff need full invoice
// creation and status-management capability -- this is their job function, so it is
// not scoped down further than the branch_manager version it is based on.
// The `invoices` table has no direct branch_id column; scoping is done by joining
// through trip_id -> trucking_trips.branch_id = the staff member's assigned branch.
...
// Note: functions.php (loaded by includes/header.php) already defines a global
// generateInvoiceNumber() -- use a distinct name here to avoid a redeclare fatal error.
function generateStaffInvoiceNumber(PDO $pdo, int $tenantId): string {
    $prefix = 'INV-' . date('Ymd') . '-';
    do {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if (!in_array($r['status'], ['paid', 'cancelled'], true)): ?>
                                ...
                                <button class="btn btn-sm btn-primary mark-status" data-id="<?= (int)$r['id'] ?>" data-status="paid" title="Mark Paid"><i class="fas fa-check-circle"></i></button>
    ...
    if ($action === 'create_invoice') {
        ...
        try {
            $number = generateStaffInvoiceNumber($pdo, $tenant_id);
            $stmt = $pdo->prepare("
    ...
    if ($action === 'update_status') {
        ...
        $check = $pdo->prepare("SELECT id, total_amount, paid_amount FROM invoices WHERE id = ? AND tenant_id = ? AND trip_id IN (SELECT id FROM trucking_trips WHERE tenant_id = ? AND branch_id = ?) LIMIT 1");
        ...
        try {
            if ($status === 'paid') {
                // Marking paid manually (outside of the Receipts workflow) settles the full balance
                $pdo->prepare("UPDATE invoices SET status = ?, paid_amount = total_amount, updated_at = NOW() WHERE id = ? AND tenant_id = ?")
                    ->execute([$status, $id, $tenant_id]);
            } else {
...
<div class="page-wrap">
    <div class="hero">
        <div>
            ...
            <small>Branch: <?= h($branch_name) ?> &middot; Trip-linked invoices only</small>
        </div>
