<?php
// tenant_admin/drivers.php
// Driver Management for Cargo Management System - Tenant Admin
    ...
    switch ($action) {
        ...
        case 'get_driver':
            ...
            if ($driver_id > 0) {
                ...
                $stmt = safeQuery($pdo, $sql, [$driver_id, $session_tenant_id]);
        ...
        case 'create_driver':
            ...
            if (!empty($license_number)) {
                $check_sql = "SELECT id FROM drivers WHERE license_number = ? AND tenant_id = ?";
                $check_stmt = safeQuery($pdo, $check_sql, [$license_number, $session_tenant_id]);
            ...
            $sql = "INSERT INTO drivers (tenant_id, user_id, full_name, phone, license_number, license_expiry, 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), ?)";
            $stmt = safeQuery($pdo, $sql, [$session_tenant_id, $user_id_assigned, $full_name, $phone, $license_number, $license_expiry, 
                    $employee_id, $hire_date, $salary_type, $salary_amount, $user_id]);
            
            if ($stmt) {
                $response = ['success' => true, 'message' => 'Driver created successfully'];
            } else {
        ...
        case 'update_driver':
            ...
            $sql = "UPDATE drivers SET full_name = ?, phone = ?, license_number = ?, license_expiry = ?, 
                    employee_id = ?, salary_type = ?, salary_amount = ?, user_id = ? 
                    WHERE id = ? AND tenant_id = ?";
            $stmt = safeQuery($pdo, $sql, [$full_name, $phone, $license_number, $license_expiry, 
                    $employee_id, $salary_type, $salary_amount, $user_id_assigned, $driver_id, $session_tenant_id]);
        ...
        case 'delete_driver':
            ...
            $check_sql = "SELECT COUNT(*) as count FROM trucking_trips WHERE driver_id = ? AND tenant_id = ? AND status NOT IN ('completed', 'delivered', 'cancelled')";
            $check_stmt = safeQuery($pdo, $check_sql, [$driver_id, $session_tenant_id]);
            ...
            $sql = "DELETE FROM drivers WHERE id = ? AND tenant_id = ?";
            $stmt = safeQuery($pdo, $sql, [$driver_id, $session_tenant_id]);
        ...
        case 'toggle_status':
            ...
            $sql = "UPDATE drivers SET is_active = ? WHERE id = ? AND tenant_id = ?";
            $stmt = safeQuery($pdo, $sql, [$is_active, $driver_id, $session_tenant_id]);
        ...
        case 'get_trips':
            ...
            $sql = "SELECT t.*, c.container_number, c.origin 
                    ...
                    WHERE t.driver_id = ? AND t.tenant_id = ?
                    ORDER BY t.created_at DESC LIMIT ?";
            $stmt = safeQuery($pdo, $sql, [$driver_id, $session_tenant_id, $limit]);
        ...
        case 'get_assignments':
            ...
            $sql = "SELECT a.*, t.trip_number, t.container_id, c.container_number 
                    ...
                    LEFT JOIN trucking_trips t ON a.trip_id = t.id
                    LEFT JOIN containers c ON t.container_id = c.id
                    WHERE a.assigned_to_driver_id = ? AND a.tenant_id = ?
        ...
        case 'get_performance':
            ...
            // Get driver user_id
            $driver_sql = "SELECT user_id FROM drivers WHERE id = ? AND tenant_id = ?";
            $driver_stmt = safeQuery($pdo, $driver_sql, [$driver_id, $session_tenant_id]);
        ...
        case 'get_users':
            $sql = "SELECT id, full_name, email, phone FROM users WHERE role IN ('driver', 'staff') AND tenant_id = ? ORDER BY full_name";
            $stmt = safeQuery($pdo, $sql, [$session_tenant_id]);
        ...
        case 'get_available_drivers':
            ...
            $sql = "SELECT d.* 
                    ...
                    AND d.id NOT IN (
                        ...
                        WHERE (DATE(departed_at) = ? OR DATE(arrived_at) = ?) AND tenant_id = ?
                    )
                    ORDER BY d.rating DESC, d.full_name";
            $stmt = safeQuery($pdo, $sql, [$session_tenant_id, $date, $date, $session_tenant_id]);
        ...
        case 'get_license_expiring':
            ...
            $stmt = safeQuery($pdo, $sql, [$days, $session_tenant_id]);
            $drivers = $stmt ? $stmt->fetchAll(PDO::FETCH_ASSOC) : [];
            $response = ['success' => true, 'data' => $drivers];
            break;
...
<head>
    ...
    <title>Driver Management - <?= htmlspecialchars($tenant_name) ?> | Cargo Management System</title>
...
<div class="container-fluid" style="padding: 20px;">
    ...
    <div class="page-header">
        <div>
            <h1><i class="fas fa-users"></i> Driver Management</h1>
