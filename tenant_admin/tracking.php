<?php
// tenant_admin/tracking.php
// Tracking for Cargo Management System - Tenant Admin
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($action === 'track_container') {
        ...
        // Search in containers (tenant_admin only sees their own containers)
        $stmt = $pdo->prepare("
            SELECT c.*, t.name as tenant_name 
            FROM containers c
            LEFT JOIN tenants t ON c.tenant_id = t.id
            WHERE (c.tracking_number = ? OR c.container_number = ?) AND c.tenant_id = ?
            LIMIT 1
        ");
        $stmt->execute([$tracking_number, $tracking_number, $session_tenant_id]);
        $container = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($container) {
            echo json_encode(['success' => true, 'type' => 'container', 'data' => $container]);
        } else {
            // Search in shipments (tenant_admin only sees their own shipments)
            $stmt = $pdo->prepare("
                SELECT t.*, 
                       c.container_number,
                       tr.truck_number,
    ...
    elseif ($action === 'get_recent_tracking') {
        // Get recent containers for this tenant
        $stmt = $pdo->prepare("
            SELECT c.*, t.name as tenant_name 
            FROM containers c
            LEFT JOIN tenants t ON c.tenant_id = t.id
            WHERE c.tracking_number IS NOT NULL AND c.tracking_number != '' AND c.tenant_id = ?
        ...
        // Get recent shipments for this tenant
        $stmt2 = $pdo->prepare("
            ...
            LIMIT 10
        ");
        $stmt2->execute([$session_tenant_id]);
...
<head>
    ...
    <title>Tracking - <?= htmlspecialchars($tenant_name) ?> | Cargo Management System</title>
...
<div class="container-fluid" style="padding: 20px;">
    ...
    <div class="page-header">
        <h1><i class="fas fa-map-marker-alt"></i> Tracking</h1>
    ...
    <div class="stats-grid">
        <div class="stat-card-sm">
            <div class="stat-info"><h4>Total</h4><div class="stat-number" id="stat-total">0</div></div>
            <div class="stat-icon"><i class="fas fa-box"></i></div>
        </div>
        <div class="stat-card-sm">
            <div class="stat-info"><h4>Received</h4><div class="stat-number" id="stat-received">0</div></div>
            <div class="stat-icon"><i class="fas fa-download"></i></div>
        </div>
        <div class="stat-card-sm">
            <div class="stat-info"><h4>Loading</h4><div class="stat-number" id="stat-loading">0</div></div>
            <div class="stat-icon"><i class="fas fa-spinner"></i></div>
        </div>
        <div class="stat-card-sm">
            <div class="stat-info"><h4>Loaded</h4><div class="stat-number" id="stat-loaded">0</div></div>
            <div class="stat-icon"><i class="fas fa-truck-loading"></i></div>
        </div>
        <div class="stat-card-sm">
            <div class="stat-info"><h4>Dispatched</h4><div class="stat-number" id="stat-dispatched">0</div></div>
            <div class="stat-icon"><i class="fas fa-paper-plane"></i></div>
        </div>
        <div class="stat-card-sm">
            <div class="stat-info"><h4>At Port</h4><div class="stat-number" id="stat-at_port">0</div></div>
            <div class="stat-icon"><i class="fas fa-ship"></i></div>
        </div>
        <div class="stat-card-sm">
            <div class="stat-info"><h4>Ready</h4><div class="stat-number" id="stat-ready">0</div></div>
            <div class="stat-icon"><i class="fas fa-check"></i></div>
        </div>
        <div class="stat-card-sm">
            <div class="stat-info"><h4>Delivered</h4><div class="stat-number" id="stat-delivered">0</div></div>
    ...
    <div class="tracking-card">
        <div class="tracking-title">
            <i class="fas fa-search"></i> Track Container or Shipment
        </div>
...
$(document).ready(function() {
                ...
                if (data.containers.length > 0 || data.shipments.length > 0) {
                    if (data.containers.length > 0) {
                        ...
                        for (let c of data.containers) {
                            const statusNames = {
                                'received': 'Received', 'loading': 'Loading', 'loaded': 'Loaded', 'shipped': 'Shipped',
                                'dispatched': 'Dispatched', 'at_port': 'At Port', 'ready': 'Ready', 'delivered': 'Delivered'
                            };
                            const statusColors = {
                                'received': '#17a2b8', 'loading': '#ffc107', 'loaded': '#fd7e14', 'shipped': '#6f42c1',
                    ...
                    html = '<div class="empty-state"><i class="fas fa-box-open"></i><p>No recent activity found</p></div>';
                }
    ...
    function trackItem() {
        ...
        $.ajax({
            ...
            error: function() {
                $('#trackResult').html('<div class="result-card"><div class="alert alert-error mb-0">Error occurred. Please try again.</div></div>');
            }
        });
    }
    ...
    function displayContainerResult(container) {
        ...
        const originNames = { 'china_yiwu': 'China (Yiwu) 🇨🇳', 'china_guangzhou': 'China (Guangzhou) 🇨🇳', 'dubai': 'Dubai 🇦🇪' };
        
        const statusOrder = ['received', 'loading', 'loaded', 'shipped', 'dispatched', 'at_port', 'ready', 'delivered'];
            ...
            <div class="result-card">
                <div class="result-header">
                    ...
                    <div>
                        ...
                        <span class="readonly-badge" style="margin-left: 10px;"><i class="fas fa-eye"></i> Read Only</span>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="info-row"><div class="info-label">Origin:</div><div class="info-value">${originNames[container.origin] || container.origin}</div></div>
                        <div class="info-row"><div class="info-label">Size (CBM):</div><div class="info-value">${container.size_cbm} CBM</div></div>
                        <div class="info-row"><div class="info-label">Weight (KG):</div><div class="info-value">${container.weight_kg} kg</div></div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <div class="info-row"><div class="info-label">Departure Date:</div><div class="info-value">${container.departure_date || '-'}</div></div>
                ...
                <div class="mt-4">
                    <h6><i class="fas fa-chart-line"></i> Journey Timeline</h6>
                    ${timelineHtml}
                </div>
                
                <div class="mt-3 text-center">
                    <small class="text-muted"><i class="fas fa-info-circle"></i> This information is read-only. Changes can be made in the management pages.</small>
                </div>
            </div>
        `;
    ...
    function displayShipmentResult(shipment) {
        ...
        const html = `
            <div class="result-card">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="info-row"><div class="info-label">Trip Number:</div><div class="info-value"><strong>${escapeHtml(shipment.trip_number)}</strong></div></div>
                        <div class="info-row"><div class="info-label">Container:</div><div class="info-value">${escapeHtml(shipment.container_number || '-')}</div></div>
                        <div class="info-row"><div class="info-label">Driver:</div><div class="info-value">${escapeHtml(shipment.driver_name || '-')}</div></div>
                        <div class="info-row"><div class="info-label">Loader:</div><div class="info-value">${escapeHtml(shipment.loader_name || '-')}</div></div>
                        <div class="info-row"><div class="info-label">Truck:</div><div class="info-value">${escapeHtml(shipment.truck_number || '-')}</div></div>
                    </div>
                    <div class="col-md-6">
                        <div class="info-row"><div class="info-label">Pickup Location:</div><div class="info-value">${escapeHtml(shipment.pickup_location || '-')}</div></div>
                        <div class="info-row"><div class="info-label">Dropoff Location:</div><div class="info-value">${escapeHtml(shipment.dropoff_location || '-')}</div></div>
                        <div class="info-row"><div class="info-label">Total CBM:</div><div class="info-value">${shipment.total_cbm} CBM</div></div>
                        <div class="info-row"><div class="info-label">Total Weight:</div><div class="info-value">${shipment.total_weight_kg} kg</div></div>
                        <div class="info-row"><div class="info-label">Created:</div><div class="info-value">${new Date(shipment.created_at).toLocaleString()}</div></div>
                    </div>
                </div>
                
                ${shipment.notes ? `<div class="info-row mt-3"><div class="info-label">Notes:</div><div class="info-value">${escapeHtml(shipment.notes)}</div></div>` : ''}
