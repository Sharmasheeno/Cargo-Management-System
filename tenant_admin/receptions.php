...
// Check Apache/PHP error log if a database error appears.
...
// Reusable WhatsApp API files. Use require_once in any page that needs WhatsApp.
$greenApiConfigFile = __DIR__ . '/../config/greenapi_connect.php';
$whatsAppHelperFile = __DIR__ . '/../includes/whatsapp_helper.php';
if (file_exists($greenApiConfigFile)) { require_once $greenApiConfigFile; }
if (file_exists($whatsAppHelperFile)) { require_once $whatsAppHelperFile; }
...
// Fixed rule: 100 points = $1, so 1 point = $0.01
...
function createBankAccount(PDO $pdo, int $tenant_id, int $user_id, array $data): array {
    ...
    // If this is default, remove default from other accounts
    if ($is_default) {
...
function validateReceiptInput(array $post, PDO $pdo, int $tenant_id, ?int $receipt_id, float $point_money_value): array {
    ...
    // Every receipt must select the account where this money will be stored.
    if (!$bank_account_id) {
...
function customerBalance(PDO $pdo, int $tenant_id, int $customer_id): float {
    $stmt = $pdo->prepare("\n        SELECT COALESCE(SUM(COALESCE(total_amount,0) - COALESCE(paid_amount,0)),0) AS balance\n        FROM invoices\n        WHERE tenant_id = ? AND customer_id = ? AND status NOT IN ('paid','cancelled')\n    ");
    $stmt->execute([$tenant_id, $customer_id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    return round(max(0, (float)($row['balance'] ?? 0)), 2);
}

function searchCustomers(PDO $pdo, int $tenant_id, string $q, int $limit = 12): array {
    $q = trim($q);
    $like = '%' . $q . '%';
    $stmt = $pdo->prepare("\n        SELECT c.id, c.customer_name, c.phone, c.email, COALESCE(c.loyalty_points,0) AS loyalty_points,\n               COALESCE(SUM(CASE WHEN i.status NOT IN ('paid','cancelled') THEN COALESCE(i.total_amount,0) - COALESCE(i.paid_amount,0) ELSE 0 END),0) AS balance\n        FROM customers c\n        LEFT JOIN invoices i ON i.customer_id = c.id AND i.tenant_id = c.tenant_id\n        WHERE c.tenant_id = ? AND COALESCE(c.is_active,1) = 1\n          AND (? = '' OR c.customer_name LIKE ? OR c.phone LIKE ? OR c.email LIKE ?)\n        GROUP BY c.id, c.customer_name, c.phone, c.email, c.loyalty_points\n        ORDER BY c.customer_name ASC\n        LIMIT $limit\n    ");
    $stmt->execute([$tenant_id, $q, $like, $like, $like]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

function createCustomerFromAjax(PDO $pdo, int $tenant_id, array $post): array {
    $name = trim($post['customer_name'] ?? '');
    $phone = trim($post['phone'] ?? '');
    $email = trim($post['email'] ?? '');
    $address = trim($post['address'] ?? '');

    if ($name === '') { throw new Exception('Customer name waa qasab'); }
    if ($phone === '') { throw new Exception('Phone number waa qasab'); }

    $cols = ['tenant_id','customer_name','phone'];
    $vals = [$tenant_id, $name, $phone];
    $marks = ['?','?','?'];

    foreach (['email' => $email, 'address' => $address] as $col => $val) {
        if (columnExists($pdo, 'customers', $col)) { $cols[] = $col; $vals[] = $val; $marks[] = '?'; }
    }
    if (columnExists($pdo, 'customers', 'is_active')) { $cols[] = 'is_active'; $vals[] = 1; $marks[] = '?'; }
    if (columnExists($pdo, 'customers', 'created_at')) { $cols[] = 'created_at'; $marks[] = 'NOW()'; }
    ...
    return ['id'=>$id, 'customer_name'=>$name, 'phone'=>$phone, 'email'=>$email, 'loyalty_points'=>0, 'balance'=>0];
}

function sendReceiptWhatsAppIfPossible(PDO $pdo, int $receipt_id, int $tenant_id): void {
    if (!function_exists('sendWhatsAppMessage')) { return; }
    $receipt = getReceipt($pdo, $receipt_id, $tenant_id);
    if (!$receipt || empty($receipt['customer_phone'])) { return; }

    $customerName = $receipt['customer_name'] ?: 'Macaamiil';
    $amount = money2($receipt['amount'] ?? 0);
    $date = $receipt['payment_date'] ?: date('Y-m-d');
    $receiptNo = $receipt['receipt_number'] ?? '';
    $message  = "Macmiil {$customerName}\n";
    $message .= "Payment received\n";
    $message .= "Amount: $" . $amount . "\n";
    $message .= "Date: {$date}\n";
    $message .= "Ref: {$receiptNo}";

    try { sendWhatsAppMessage($receipt['customer_phone'], $message); } catch (Throwable $e) { error_log('WhatsApp receipt send failed: ' . $e->getMessage()); }
}

function exportReceiptsCsv(PDO $pdo, int $tenant_id): void {
    $stmt = $pdo->prepare("\n        SELECT r.receipt_number, r.payment_date, c.customer_name, c.phone, r.original_amount,\n               r.discount_applied, r.points_used, r.points_discount_amount, r.amount,\n               r.points_earned, r.payment_method, r.reference_number, ba.account_name, r.notes\n        FROM receipts r\n        LEFT JOIN customers c ON c.id = r.customer_id\n        LEFT JOIN bank_accounts ba ON ba.id = r.bank_account_id\n        WHERE r.tenant_id = ?\n        ORDER BY r.created_at DESC, r.id DESC\n    ");
    $stmt->execute([$tenant_id]);
    if (ob_get_length()) { ob_clean(); }
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="receipts_export_' . date('Ymd_His') . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Receipt No','Date','Customer','Phone','Original','Discount','Points Used','Points Discount','Final Paid','Points Earned','Method','Reference','Account','Notes']);
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { fputcsv($out, $row); }
    fclose($out);
    exit;
}

function importReceiptsCsv(PDO $pdo, int $tenant_id, int $user_id, array $file, float $loyalty_rate, float $point_money_value): array {
    ...
    $defaultAccount = getBankAccounts($pdo, $tenant_id)[0]['id'] ?? null;
    if (!$defaultAccount) { throw new Exception('Import wuxuu u baahan yahay ugu yaraan hal bank/cash account'); }

    while (($row = fgetcsv($handle)) !== false) {
        try {
            $name = trim($row[2] ?? ''); $phone = trim($row[3] ?? '');
            if ($name === '' || $phone === '') { $skipped++; continue; }
            $found = searchCustomers($pdo, $tenant_id, $phone, 1);
            if (!$found) { $customer = createCustomerFromAjax($pdo, $tenant_id, ['customer_name'=>$name,'phone'=>$phone]); }
            else { $customer = $found[0]; }
            $data = [
                'customer_id' => (int)$customer['id'], 'invoice_id'=>null, 'payment_id'=>null,
                'original_amount'=>(float)($row[4] ?? $row[8] ?? 0), 'discount_applied'=>(float)($row[5] ?? 0),
                'points_used'=>(float)($row[6] ?? 0), 'points_discount_amount'=>(float)($row[7] ?? 0),
                'amount'=>(float)($row[8] ?? 0), 'payment_method'=>$row[10] ?? 'cash',
                'reference_number'=>$row[11] ?? '', 'bank_account_id'=>(int)$defaultAccount,
                'payment_date'=>!empty($row[1]) ? date('Y-m-d', strtotime($row[1])) : date('Y-m-d'),
                'notes'=>$row[13] ?? ''
            ];
            createReceipt($pdo, $tenant_id, $user_id, $data, $loyalty_rate, $point_money_value);
            $imported++;
        } catch (Throwable $e) { $errors[] = $e->getMessage(); $skipped++; }
    }
    fclose($handle);
    return ['success'=>true, 'message'=>"Import done: {$imported} imported, {$skipped} skipped", 'errors'=>$errors];
}
...
function createReceipt(PDO $pdo, int $tenant_id, int $user_id, array $data, float $loyalty_rate, float $point_money_value): array {
    ...
    try {
        ...
        // Store/deposit this receipt amount into the selected account.
        ...
        sendReceiptWhatsAppIfPossible($pdo, $receipt_id, $tenant_id);
...
function updateReceipt(PDO $pdo, int $tenant_id, int $user_id, int $receipt_id, array $data, float $loyalty_rate, float $point_money_value): array {
    ...
    try {
        ...
        // Remove old receipt amount from its old account, then add the new amount to the selected account.
...
function deleteReceipt(PDO $pdo, int $tenant_id, int $receipt_id): array {
    ...
    try {
        ...
        // Remove deleted receipt amount from the selected account balance.
        updateBankAccountBalance($pdo, $tenant_id, !empty($existing['bank_account_id']) ? (int)$existing['bank_account_id'] : null, -1 * (float)$existing['amount']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    try {
        if ($action === 'get_customers' || $action === 'search_customers') {
            $q = trim($_POST['q'] ?? '');
            json_response(['success' => true, 'customers' => searchCustomers($pdo, $user_tenant_id, $q, 20)]);
        }

        if ($action === 'create_customer') {
            $customer = createCustomerFromAjax($pdo, $user_tenant_id, $_POST);
            json_response(['success' => true, 'customer' => $customer, 'message' => 'Customer waa la abuuray']);
        }

        if ($action === 'export_receipts') {
            exportReceiptsCsv($pdo, $user_tenant_id);
        }

        if ($action === 'import_receipts') {
            json_response(importReceiptsCsv($pdo, $user_tenant_id, $user_id, $_FILES['import_file'] ?? [], $tenant_loyalty_rate, $tenant_point_money_value));
        }
        ...
        if ($action === 'create_receipt') {
            ...
            json_response(createReceipt($pdo, $user_tenant_id, $user_id, $data, $tenant_loyalty_rate, $tenant_point_money_value));
        }
        ...
        if ($action === 'update_receipt') {
            ...
            json_response(updateReceipt($pdo, $user_tenant_id, $user_id, $receipt_id, $data, $tenant_loyalty_rate, $tenant_point_money_value));
        }
                        ...
                        <?php foreach ($receipts as $r): ?>
                            <tr>
                                ...
                                <td class="action-buttons">
                                    <button class="btn btn-sm btn-info view-receipt" data-id="<?= (int)$r['id'] ?>"><i class="fas fa-eye"></i></button>
                                    <button class="btn btn-sm btn-warning edit-receipt" data-id="<?= (int)$r['id'] ?>"><i class="fas fa-edit"></i></button>
                                    <button class="btn btn-sm btn-danger delete-receipt" data-id="<?= (int)$r['id'] ?>"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
...
<head>
    ...
    <style>
        ...
        .customer-results { display:none; position:absolute; top:100%; left:0; right:0; z-index:3000; background:#fff; border:1px solid #ddd; border-radius:10px; max-height:260px; overflow:auto; box-shadow:0 10px 30px rgba(0,0,0,.12); }
        .customer-result-item, .customer-add-item { padding:10px 12px; cursor:pointer; border-bottom:1px solid #f1f1f1; }
        .customer-result-item:hover, .customer-add-item:hover { background:#f7f3fb; }
        .customer-result-name { font-weight:700; color:#111827; }
        .customer-result-meta { font-size:12px; color:#6b7280; }
    </style>
</head>
<body>
...
<div class="page-wrap">
    ...
    <div class="page-header-custom">
        <div>
            ...
            <small class="text-muted">Insert, update, delete + loyalty points + bank accounts</small>
        </div>
        ...
        <div class="d-flex align-items-center flex-wrap" style="gap:8px;">
            ...
            <button class="btn btn-outline-success" id="exportBtn"><i class="fas fa-file-export"></i> Export</button>
            <button class="btn btn-outline-info" id="importBtn"><i class="fas fa-file-import"></i> Import</button>
            <button class="btn-primary-custom" id="addReceiptBtn"><i class="fas fa-plus-circle"></i> Rasiid Cusub</button>
        </div>
    </div>
    ...
    <div class="filters-card">
        <div class="filter-form">
            ...
            <div class="filter-group">
                ...
                <div class="customer-search" data-target="#customerFilter" data-mode="filter">
                    <input type="text" class="form-control customer-search-input" placeholder="Search customer name ama phone...">
                    <div class="customer-results"></div>
                    <div class="customer-selected text-muted small mt-1">Dhammaan customers</div>
                </div>
            </div>
            <div>
                <button class="btn btn-secondary" id="resetBtn"><i class="fas fa-undo"></i> Nadiifi</button>
                <button class="btn-primary-custom" id="refreshBtn"><i class="fas fa-sync"></i> Refresh</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-lg">
        <form class="modal-content" id="receiptForm">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-6 form-group">
                        ...
                        <div class="customer-search" data-target="#customer_id" data-mode="form">
                            <input type="text" class="form-control customer-search-input" placeholder="Search customer name ama phone...">
                            <div class="customer-results"></div>
                            <div class="customer-selected text-muted small mt-1">Customer lama dooran</div>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-primary-custom"><i class="fas fa-save"></i> Kaydi & Points Bixi Automatic</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal fade" id="addCustomerModal" tabindex="-1">
    <div class="modal-dialog">
        <form class="modal-content" id="addCustomerForm">
            <div class="modal-header" style="background:#2D1859;color:#fff;">
                <h5 class="modal-title"><i class="fas fa-user-plus"></i> Add Customer</h5>
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <input type="hidden" id="add_customer_target">
                <div class="form-group"><label>Name *</label><input type="text" name="customer_name" id="new_customer_name" class="form-control" required></div>
                <div class="form-group"><label>Phone *</label><input type="text" name="phone" id="new_customer_phone" class="form-control" required></div>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <form class="modal-content" id="importForm" enctype="multipart/form-data">
            <div class="modal-header" style="background:#2D1859;color:#fff;">
                <h5 class="modal-title"><i class="fas fa-file-import"></i> Import Receipts CSV</h5>
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="alert alert-info">CSV format: Receipt No, Date, Customer, Phone, Original, Discount, Points Used, Points Discount, Final Paid, Points Earned, Method, Reference, Account, Notes.</div>
                <input type="file" name="import_file" class="form-control" accept=".csv,text/csv" required>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Xidh</button>
                <button type="submit" class="btn-primary-custom">Import</button>
            </div>
        </form>
    </div>
</div>
...
function setSelectedCustomerPoints(extraPoints = 0) {
    const points = Number(selectedCustomerPoints || 0) + Number(extraPoints || 0);
    $('#available_points_text').text('Available: ' + fixed2(points) + ' pts');
}
...
function loadBankAccounts(callback) {
    $.ajax({
        ...
        if (res && res.success) {
            ...
            // Update bank account dropdown in receipt form
            ...
            // Update bank accounts list in management modal
...
function renderCustomerResults($box, customers, query) {
    const $results = $box.find('.customer-results');
    if (!customers.length) {
        $results.html(`<div class="customer-add-item" data-query="${escapeHtml(query)}"><i class="fas fa-plus-circle"></i> Add Customer "${escapeHtml(query)}"</div>`).show();
        return;
    }
    let html = '';
    customers.forEach(c => {
        html += `<div class="customer-result-item" data-id="${c.id}" data-name="${escapeHtml(c.customer_name)}" data-phone="${escapeHtml(c.phone || '')}" data-balance="${fixed2(c.balance)}" data-points="${fixed2(c.loyalty_points)}">
            <div class="customer-result-name">${escapeHtml(c.customer_name)}</div>
            <div class="customer-result-meta"><i class="fas fa-phone"></i> ${escapeHtml(c.phone || '-')} | Balance: $${fixed2(c.balance)} | Points: ${fixed2(c.loyalty_points)}</div>
        </div>`;
    });
    html += `<div class="customer-add-item" data-query="${escapeHtml(query)}"><i class="fas fa-plus-circle"></i> Add Customer</div>`;
    $results.html(html).show();
}

function selectCustomer($box, customer) {
    const target = $box.data('target');
    const mode = $box.data('mode');
    $(target).val(customer.id).trigger('change');
    $box.find('.customer-search-input').val(customer.customer_name + (customer.phone ? ' - ' + customer.phone : ''));
    $box.find('.customer-selected').html(`Selected: <strong>${escapeHtml(customer.customer_name)}</strong> | Phone: ${escapeHtml(customer.phone || '-')} | Balance: $${fixed2(customer.balance)} | Points: ${fixed2(customer.loyalty_points)}`);
    $box.find('.customer-results').hide();

    if (mode === 'form') {
        selectedCustomerPoints = Number(customer.loyalty_points || 0);
...
function searchCustomerBox($box, query) {
    $.ajax({
        url: window.location.href,
        type: 'POST',
        dataType: 'json',
        data: {ajax_action:'search_customers', q: query || ''}
    }).done(function(res){
        const customers = (res && res.success) ? (res.customers || []) : [];
        customersCache = customers;
        renderCustomerResults($box, customers, query || '');
    });
}
...
function initCustomerSearch() {
    $('.customer-search').each(function(){
        const $box = $(this);
        let timer = null;
        $box.find('.customer-search-input').off('input focus').on('input focus', function(){
            const q = $(this).val();
            clearTimeout(timer);
            timer = setTimeout(() => searchCustomerBox($box, q), 250);
        });
    });

    $(document).on('click', '.customer-result-item', function(){
        const $item = $(this);
        const $box = $item.closest('.customer-search');
        selectCustomer($box, {
            id: $item.data('id'),
            customer_name: $item.data('name'),
            phone: $item.data('phone'),
            balance: $item.data('balance'),
            loyalty_points: $item.data('points')
        });
    });

    $(document).on('click', '.customer-add-item', function(){
        const $box = $(this).closest('.customer-search');
        $('#add_customer_target').val($box.data('target'));
        $('#new_customer_name').val($(this).data('query') || '');
        $('#new_customer_phone').val('');
        $('#addCustomerModal').modal('show');
    });

    $(document).on('click', function(e){
        if (!$(e.target).closest('.customer-search').length) $('.customer-results').hide();
    });
}

function loadCustomers(callback) {
    $('.customer-search[data-mode="filter"] .customer-selected').text('Dhammaan customers');
    if (callback) callback();
}
...
function resetForm() {
    ...
    $('#bank_account_id').val('');
    $('.customer-search[data-mode="form"] .customer-search-input').val('');
    $('.customer-search[data-mode="form"] .customer-selected').text('Customer lama dooran');
}
...
function bindTableEvents() {
    ...
    $('.edit-receipt').off('click').on('click', function() {
        ...
        $.post(window.location.href, {ajax_action: 'get_receipt_data', receipt_id: id}, function(res) {
            ...
            selectedCustomerPoints = Number(r.loyalty_points || 0);
            const $formBox = $('.customer-search[data-target="#customer_id"]');
            $formBox.find('.customer-search-input').val((r.customer_name || '') + (r.customer_phone ? ' - ' + r.customer_phone : ''));
            $formBox.find('.customer-selected').html(`Selected: <strong>${escapeHtml(r.customer_name || '-')}</strong> | Phone: ${escapeHtml(r.customer_phone || '-')} | Points: ${fixed2(r.loyalty_points || 0)}`);
...
// Customer is selected through AJAX search, not through a select box.
    ...
    $('#exportBtn').on('click', function() {
        const form = $('<form method="POST" style="display:none"><input name="ajax_action" value="export_receipts"></form>');
        $('body').append(form); form.submit(); form.remove();
    });

    $('#importBtn').on('click', function() { $('#importModal').modal('show'); });

    $('#importForm').on('submit', function(e) {
        e.preventDefault();
        const fd = new FormData(this);
        fd.append('ajax_action', 'import_receipts');
        $.ajax({url: window.location.href, type:'POST', data:fd, processData:false, contentType:false, dataType:'json'})
        .done(function(res){
            if (res.success) { showAlert('success', res.message || 'Imported'); $('#importModal').modal('hide'); loadReceipts(); }
            else showAlert('error', res.message || 'Import failed');
        }).fail(function(xhr){ showAlert('error', 'Import failed: ' + (xhr.responseText || 'server error')); });
    });

    $('#addCustomerForm').on('submit', function(e) {
        e.preventDefault();
        const fd = $(this).serialize() + '&ajax_action=create_customer';
        $.post(window.location.href, fd, function(res){
            if (res.success) {
                const target = $('#add_customer_target').val();
                const $box = $('.customer-search[data-target="' + target + '"]');
                selectCustomer($box, res.customer);
                $('#addCustomerModal').modal('hide');
                showAlert('success', res.message || 'Customer saved');
            } else showAlert('error', res.message || 'Customer save failed');
        }, 'json');
    });
