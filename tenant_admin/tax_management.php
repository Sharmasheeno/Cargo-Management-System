<?php
// tenant_admin/tax_management.php
// Tax Management & Tax Reporting for Cargo Management System - Tenant Admin
...
try {
    ...
    // Insert default tax rates for this tenant
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM tax_rates WHERE tenant_id = ?");
    $stmt->execute([$session_tenant_id]);
    if ($stmt->fetchColumn() == 0) {
        ...
        $stmt = $pdo->prepare("INSERT INTO tax_rates (tenant_id, tax_name, tax_rate, tax_type, notes, is_default) VALUES (?, ?, ?, ?, ?, ?)");
        foreach ($default_taxes as $tax) {
            $stmt->execute([$session_tenant_id, $tax[0], $tax[1], $tax[2], $tax[3], $tax[4]]);
        }
    }
    
    // Initialize tax settings for this tenant
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM tax_settings WHERE tenant_id = ?");
    $stmt->execute([$session_tenant_id]);
    if ($stmt->fetchColumn() == 0) {
        $default_tax = $pdo->prepare("SELECT id FROM tax_rates WHERE tenant_id = ? AND tax_type = 'VAT' LIMIT 1");
        $default_tax->execute([$session_tenant_id]);
        $default_tax_id = $default_tax->fetch(PDO::FETCH_ASSOC);
        $default_tax_id = $default_tax_id ? $default_tax_id['id'] : null;
        ...
        $stmt->execute([$session_tenant_id, $default_tax_id]);
    }
...
// Get tax rates for this tenant
$tax_rates = [];
try {
    $stmt = $pdo->prepare("SELECT tr.* FROM tax_rates tr WHERE tr.tenant_id = ? ORDER BY tr.tax_type, tr.tax_name");
    $stmt->execute([$session_tenant_id]);
...
// Get tax settings for this tenant
$tax_settings = [];
try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($action === 'save_tax_rate') {
        ...
        try {
            ...
            if (empty($id)) {
                ...
                // Verify tax rate belongs to this tenant
                $check = $pdo->prepare("SELECT id FROM tax_rates WHERE id = ? AND tenant_id = ?");
                $check->execute([$id, $tenant_id]);
                if (!$check->fetch()) {
                    echo json_encode(['success' => false, 'message' => 'Tax rate not found or you do not have permission']);
                    exit;
                }
                $stmt = $pdo->prepare("UPDATE tax_rates SET tax_name=?, tax_rate=?, tax_type=?, tax_number=?, is_default=?, is_active=?, effective_from=?, effective_to=?, notes=? WHERE id=? AND tenant_id=?");
                $stmt->execute([$tax_name, $tax_rate, $tax_type, $tax_number, $is_default, $is_active, $effective_from, $effective_to, $notes, $id, $tenant_id]);
    ...
    if ($action === 'get_tax_rate') {
        $id = $_POST['id'] ?? 0;
        $stmt = $pdo->prepare("SELECT * FROM tax_rates WHERE id = ? AND tenant_id = ?");
        $stmt->execute([$id, $session_tenant_id]);
    ...
    if ($action === 'set_default_tax') {
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE tax_rates SET is_default = 0 WHERE tenant_id = ?");
            $stmt->execute([$session_tenant_id]);
            $stmt = $pdo->prepare("UPDATE tax_rates SET is_default = 1 WHERE id = ? AND tenant_id = ?");
            $stmt->execute([$id, $session_tenant_id]);
    ...
    if ($action === 'get_tax_returns') {
        ...
        $sql = "SELECT tr.*, tax_rate.tax_name, tax_rate.tax_rate, tax_rate.tax_type
                FROM tax_returns tr
    ...
    if ($action === 'save_tax_return') {
        ...
        // Verify tax rate belongs to this tenant
        $check = $pdo->prepare("SELECT id FROM tax_rates WHERE id = ? AND tenant_id = ?");
        $check->execute([$tax_rate_id, $tenant_id]);
        if (!$check->fetch()) {
            echo json_encode(['success' => false, 'message' => 'Tax rate not found or you do not have permission']);
            exit;
        }
        ...
        try {
            if (empty($id)) {
                ...
                // Verify return belongs to this tenant
                $checkReturn = $pdo->prepare("SELECT id FROM tax_returns WHERE id = ? AND tenant_id = ?");
                $checkReturn->execute([$id, $tenant_id]);
                if (!$checkReturn->fetch()) {
                    echo json_encode(['success' => false, 'message' => 'Tax return not found or you do not have permission']);
                    exit;
                }
                $stmt = $pdo->prepare("UPDATE tax_returns SET tax_rate_id=?, return_period=?, return_year=?, return_month=?, return_quarter=?, filing_date=?, due_date=?, taxable_amount=?, tax_amount=?, penalties=?, interest=?, total_due=?, notes=?, status=? WHERE id=? AND tenant_id=?");
                $stmt->execute([$tax_rate_id, $return_period, $return_year, $return_month, $return_quarter, $filing_date, $due_date, $taxable_amount, $tax_amount, $penalties, $interest, $total_due, $notes, $status, $id, $tenant_id]);
    ...
    if ($action === 'delete_tax_return') {
        $id = $_POST['id'] ?? 0;
        try {
            $check = $pdo->prepare("SELECT id FROM tax_returns WHERE id = ? AND tenant_id = ?");
            $check->execute([$id, $session_tenant_id]);
            if (!$check->fetch()) {
                echo json_encode(['success' => false, 'message' => 'Tax return not found']);
                exit;
            }
            $stmt = $pdo->prepare("DELETE FROM tax_returns WHERE id = ? AND tenant_id = ?");
            $stmt->execute([$id, $session_tenant_id]);
    ...
    if ($action === 'get_tax_return') {
        $id = $_POST['id'] ?? 0;
        $stmt = $pdo->prepare("SELECT * FROM tax_returns WHERE id = ? AND tenant_id = ?");
        $stmt->execute([$id, $session_tenant_id]);
        echo json_encode($stmt->fetch(PDO::FETCH_ASSOC));
        exit;
    }
    ...
    if ($action === 'record_tax_payment') {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT * FROM tax_returns WHERE id = ? AND tenant_id = ? FOR UPDATE");
            $stmt->execute([$return_id, $session_tenant_id]);
            ...
            $stmt = $pdo->prepare("UPDATE tax_returns SET amount_paid = ?, payment_reference = ?, payment_date = ?, status = ? WHERE id = ? AND tenant_id = ?");
            $stmt->execute([$new_paid, $payment_reference, $payment_date, $new_status, $return_id, $session_tenant_id]);
    ...
    if ($action === 'get_tax_report') {
        ...
        $where = ["tr.tenant_id = ?", "tr.return_year = ?"];
        $params = [$session_tenant_id, $year];
        ...
        $total_taxable = array_sum(array_column($reports, 'taxable_amount'));
        $total_tax = array_sum(array_column($reports, 'tax_amount'));
        $total_paid = array_sum(array_column($reports, 'amount_paid'));
        $total_balance = $total_tax - $total_paid;
            ...
            <div class="col-md-3">
                <div class="card bg-info text-white">
                    <div class="card-body">
                        ...
                        <h3>$<?= number_format($total_taxable, 2) ?></h3>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card bg-warning text-white">
                    <div class="card-body">
                        <h6>Total Tax Due</h6>
                        <h3>$<?= number_format($total_tax, 2) ?></h3>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $report['tax_rate'] ?>%</span></td>
                        <td>$<?= number_format($report['taxable_amount'], 2) ?></td>
                        <td>$<?= number_format($report['tax_amount'], 2) ?></span></td>
                        <td>$<?= number_format($report['penalties'], 2) ?></span></td>
    ...
    if ($action === 'get_tax_settings') {
        ...
        $stmt = $pdo->prepare("SELECT id, tax_name, tax_rate FROM tax_rates WHERE tenant_id = ? AND is_active = 1");
        $stmt->execute([$session_tenant_id]);
    ...
    if ($action === 'save_tax_settings') {
        ...
        try {
            // Verify default tax rate belongs to this tenant if set
            if ($default_tax_rate_id) {
                $check = $pdo->prepare("SELECT id FROM tax_rates WHERE id = ? AND tenant_id = ?");
                $check->execute([$default_tax_rate_id, $tenant_id]);
                if (!$check->fetch()) {
                    echo json_encode(['success' => false, 'message' => 'Invalid default tax rate selected']);
                    exit;
                }
            }
...
<head>
    <meta charset="UTF-8">
    <title>Tax Management - <?= htmlspecialchars($tenant_name) ?> | Cargo Management System</title>
    ...
    <style>
        ...
        .nav-tabs .nav-link { color: var(--curdun-violet); font-weight: 600; cursor: pointer; }
        .nav-tabs .nav-link.active { background: var(--curdun-violet); color: white; border-color: var(--curdun-violet); }
        
        .table-container { background: white; border-radius: 12px; overflow: auto; padding: 20px; }
...
<div class="container-fluid" style="padding: 20px;">
    ...
    <div class="page-header">
        <div>
            ...
            <p>Manage tax rates, file returns, and track tax payments for <?= htmlspecialchars($tenant_name) ?></p>
        </div>
    ...
    <div id="ratesTab" class="tab-content">
        <div class="table-container" id="taxRatesContainer"></div>
    </div>
    
    <!-- Tax Returns Tab -->
    <div id="returnsTab" class="tab-content" style="display: none;">
        <div class="table-container" id="taxReturnsContainer"></div>
    </div>
    
    <!-- Tax Reports Tab -->
    <div id="reportsTab" class="tab-content" style="display: none;">
        <div class="white-card p-4 bg-white rounded mb-3">
            <div class="row">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <label><i class="fas fa-tag"></i> Tax Type</label>
                ...
                <div class="col-md-2">
                    <label>&nbsp;</label>
                    <button class="btn btn-violet w-100" id="generateReport"><i class="fas fa-chart-line"></i> Generate</button>
                </div>
            </div>
        </div>
        <div id="taxReportContainer" class="table-container"></div>
    </div>
    
    <!-- Tax Settings Tab -->
    <div id="settingsTab" class="tab-content" style="display: none;">
        <div class="white-card bg-white rounded p-4">
            <h5 class="mb-3"><i class="fas fa-cog"></i> Tax Configuration</h5>
            <form id="taxSettingsForm">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label><i class="fas fa-percent"></i> Default Tax Rate</label>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label><i class="fas fa-calculator"></i> Tax Calculation Method</label>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label><i class="fas fa-chart-line"></i> Tax Period</label>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="custom-control custom-switch mt-4">
                                ...
                                <label class="custom-control-label" for="enableTaxInvoicing"><i class="fas fa-file-invoice"></i> Enable Tax Invoicing</label>
                            </div>
                        </div>
                    </div>
                </div>
                <hr>
                <h6 class="mb-3"><i class="fas fa-university"></i> Tax Authority Information</h6>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                ...
                <button type="submit" class="btn btn-violet mt-3"><i class="fas fa-save"></i> Save Settings</button>
            </form>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px;">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-percent"></i> Tax Rate Details</h5>
            ...
            <form id="taxRateForm">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label><i class="fas fa-chart-bar"></i> Tax Type</label>
                    ...
                    <div class="form-group">
                        <label><i class="fas fa-id-card"></i> Tax Number / Registration</label>
                        <input type="text" name="tax_number" id="taxNumber" class="form-control">
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label><i class="fas fa-calendar-alt"></i> Effective From</label>
                                <input type="date" name="effective_from" id="effectiveFrom" class="form-control">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label><i class="fas fa-calendar-alt"></i> Effective To</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-violet">Save Tax Rate</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="taxReturnModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px;">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-file-upload"></i> Tax Return Filing</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <form id="taxReturnForm">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label><i class="fas fa-calendar-alt"></i> Return Period *</label>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label><i class="fas fa-calendar-check"></i> Filing Date</label>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-violet">Save Return</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="taxPaymentModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px;">
            <div class="modal-header" style="background: linear-gradient(135deg, #0F7A3A, #20c997);">
                <h5 class="modal-title"><i class="fas fa-money-bill"></i> Record Tax Payment</h5>
            ...
            <form id="taxPaymentForm">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <strong><span id="paymentReturnName"></span></strong>
                    </div>
                    <div class="form-group">
                        <label>Balance Due:</label>
                        <strong class="text-danger" id="paymentBalance">$0.00</strong>
                    </div>
                    <div class="form-group">
                        <label><i class="fas fa-dollar-sign"></i> Payment Amount *</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="payment_reference" id="paymentReference" class="form-control" placeholder="Check #, Transaction ID...">
                    </div>
                    <div class="form-group">
                        <label><i class="fas fa-sticky-note"></i> Notes</label>
...
function loadTaxSettings() {
    $.post(window.location.href, {ajax_action: 'get_tax_settings'}, function(res) {
        if(res.settings) {
...
function populateTaxRatesDropdown() {
    $.post(window.location.href, {ajax_action: 'get_tax_rates'}, function(res) {
