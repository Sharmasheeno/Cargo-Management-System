<?php
// superadmin/loaders.php
//faras cargo - Complete Loader Management System
// Professional loader management with assignments, performance tracking, and scheduling
...
// Only superadmin, company_admin, warehouse_supervisor, and logistics_supervisor can access this page
if (!in_array($role, ['superadmin', 'company_admin', 'warehouse_supervisor', 'logistics_supervisor'])) {
    ...
    switch ($action) {
        case 'get_loaders':
            $sql = "SELECT l.*, 
                    ...
                    (SELECT COUNT(*) FROM assignments WHERE assigned_to_loader_id = l.id AND status IN ('completed', 'done')) as completed_tasks,
                    (SELECT COUNT(*) FROM assignments WHERE assigned_to_loader_id = l.id AND status = 'pending') as pending_tasks,
            ...
            } elseif ($role === 'warehouse_supervisor' || $role === 'logistics_supervisor') {
                $sql .= " WHERE l.tenant_id = ?";
                $params = [$session_tenant_id];
            } else {
                $params = [];
            ...
            $sql .= " ORDER BY l.is_active DESC, l.full_name ASC";
            ...
            if ($loader_id > 0) {
                ...
                if ($stmt && $row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $assign_sql = "SELECT a.*, ws.stock_name, ws.origin,
                                  ...
                                  WHERE a.assigned_to_loader_id = ? AND a.status = 'in_progress'
                                  ORDER BY a.created_at DESC LIMIT 5";
                    $assign_stmt = safeQuery($pdo, $assign_sql, [$loader_id]);
        ...
        case 'create_loader':
            ...
            if (!empty($employee_id)) {
                $check_sql = "SELECT id FROM loaders WHERE employee_id = ?";
                $params = [$employee_id];
                if ($role !== 'superadmin') {
            ...
            $sql = "INSERT INTO loaders (tenant_id, user_id, full_name, phone, employee_id, hire_date, 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), ?)";
            $stmt = safeQuery($pdo, $sql, [$tenant_id, $user_id_assigned, $full_name, $phone, $employee_id, $hire_date, 
                    $salary_type, $salary_amount, $specialization, $max_load_weight, $certifications, $user_id]);
            
            if ($stmt) {
                $loader_id = $pdo->lastInsertId();
                
                // Log activity
                logActivity($user_id, 'CREATE_LOADER', 'loaders', $loader_id, null, ['full_name' => $full_name]);
                
                $response = ['success' => true, 'message' => 'Loader created successfully', 'loader_id' => $loader_id];
            } else {
        ...
        case 'update_loader':
            ...
            $sql = "UPDATE loaders SET full_name = ?, phone = ?, employee_id = ?, 
                    ...
                    max_load_weight = ?, certifications = ? WHERE id = ?";
            $params = [$full_name, $phone, $employee_id, $salary_type, $salary_amount, 
                      $user_id_assigned, $specialization, $max_load_weight, $certifications, $loader_id];
            ...
            if ($stmt) {
                logActivity($user_id, 'UPDATE_LOADER', 'loaders', $loader_id, null, ['full_name' => $full_name]);
        ...
        case 'delete_loader':
            ...
            $check_sql = "SELECT COUNT(*) as count FROM assignments WHERE assigned_to_loader_id = ? AND status IN ('pending', 'in_progress')";
            $check_stmt = safeQuery($pdo, $check_sql, [$loader_id]);
            ...
            $sql = "DELETE FROM loaders WHERE id = ?";
            $params = [$loader_id];
            ...
            if ($stmt) {
                logActivity($user_id, 'DELETE_LOADER', 'loaders', $loader_id, null, null);
        ...
        case 'toggle_status':
            ...
            $sql = "UPDATE loaders SET is_active = ? WHERE id = ?";
            $params = [$is_active, $loader_id];
            ...
            if ($stmt) {
                ...
                logActivity($user_id, 'TOGGLE_LOADER_STATUS', 'loaders', $loader_id, null, ['status' => $status_text]);
        ...
        case 'get_assignments':
            ...
            $sql = "SELECT a.*, 
                    ...
                    FROM assignments a
                    LEFT JOIN warehouse_stock ws ON a.task_description LIKE CONCAT('%', ws.stock_name, '%')
                    WHERE a.assigned_to_loader_id = ?
                    ORDER BY a.created_at DESC LIMIT 20";
            $stmt = safeQuery($pdo, $sql, [$loader_id]);
        ...
        case 'get_performance':
            ...
            // Get loader user_id
            $loader_sql = "SELECT user_id FROM loaders WHERE id = ?";
            $loader_stmt = safeQuery($pdo, $loader_sql, [$loader_id]);
        ...
        case 'get_users':
            $sql = "SELECT id, full_name, email, phone FROM users WHERE role IN ('loader', 'staff')";
            $params = [];
        ...
        case 'get_available_loaders':
            ...
            // Exclude loaders with assignments on this date
            $sql .= " AND l.id NOT IN (
                        SELECT DISTINCT assigned_to_loader_id FROM assignments 
                        WHERE DATE(created_at) = ? AND status IN ('pending', 'in_progress')
                    )";
            $params[] = $date;
            
            $sql .= " ORDER BY l.rating DESC, l.full_name";
        ...
        case 'assign_task':
            ...
            $sql = "INSERT INTO assignments (tenant_id, assigned_to_loader_id, assigned_by, task_type, task_description, 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', NOW())";
            $tenant_id = ($role === 'superadmin') ? (isset($_POST['tenant_id']) && !empty($_POST['tenant_id']) ? (int)$_POST['tenant_id'] : null) : $session_tenant_id;
            $stmt = safeQuery($pdo, $sql, [$tenant_id, $loader_id, $user_id, $task_type, $task_description, $priority, $due_date]);
            
            if ($stmt) {
                logActivity($user_id, 'ASSIGN_TASK', 'assignments', $pdo->lastInsertId(), null, ['loader_id' => $loader_id]);
...
<head>
    ...
    <title>Loader Management - Cargo Management System</title>
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: ratings,
                        borderColor: '#2D1859',
                        backgroundColor: 'rgba(82, 0, 102, 0.1)',
                        fill: true,
                        tension: 0.4
                    }]
                },
