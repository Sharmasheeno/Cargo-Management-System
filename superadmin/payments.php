...
// Check if AccountingService exists before requiring
...
// Get all tenants for filter dropdown (only for superadmin)
$tenants = [];
if ($is_superadmin) {
    try {
...
// Get all bank accounts based on user's tenant
$bank_accounts = [];
try {
    if ($is_superadmin) {
        $stmt = $pdo->query("
 ...
 * Supplier payments do not earn points.
 */
function awardLoyaltyPointsForPayment(PDO $pdo, int $payment_id, int $created_by = 0): array
{
    ...
    if (!$payment || empty($payment['customer_id'])) {
        return ['success' => false, 'points' => 0.00, 'message' => 'Payment customer kuma xirna'];
    }
...
/**
 * Reverse loyalty points generated by a payment.
 * Used when payment is updated or deleted so points do not become duplicated/wrong.
 */
function reverseLoyaltyPointsForPayment(PDO $pdo, int $payment_id): array
{
    $stmt = $pdo->prepare("
        SELECT tenant_id, customer_id, COALESCE(SUM(points_earned), 0) AS total_points
        FROM loyalty_points_log
        WHERE reference_type = 'payment'
          AND reference_id = ?
          AND points_earned > 0
    ...
    if (!$log) {
        return ['success' => false, 'points' => 0.00, 'message' => 'No loyalty points to reverse'];
    }

    $points = round((float)$log['total_points'], 2);
    ...
    $deleteLog = $pdo->prepare("
        ...
        WHERE reference_type = 'payment'
          ...
          AND points_earned > 0
    ");
    $deleteLog->execute([$payment_id]);

    return ['success' => true, 'points' => $points, 'message' => number_format($points, 2) . ' points reversed'];
}

/**
 * Get pending redemption discount amount for a customer
 * @return float Total pending discount amount
 */
function getPendingRedemptionDiscount(PDO $pdo, int $customer_id, int $tenant_id): float
{
    try {
        $stmt = $pdo->prepare("
        ...
        error_log("Error getting pending redemption: " . $e->getMessage());
        return 0.00;
    }
}

/**
 * Get pending redemptions details for display
 */
function getPendingRedemptionsList(PDO $pdo, int $customer_id, int $tenant_id): array
{
    try {
        $stmt = $pdo->prepare("
            SELECT id, points_used, discount_amount, redemption_date
            FROM point_redemptions
            WHERE customer_id = ? AND tenant_id = ? AND status = 'pending'
...
/**
 * Apply pending redemption to a payment
 * @return array Result with success status, applied discount, and remaining payment amount
 */
function applyPendingRedemptionToPayment(PDO $pdo, int $payment_id, int $customer_id, int $tenant_id): array
{
    try {
        // Get payment amount
        $stmt = $pdo->prepare("
            SELECT amount, original_amount, discount_applied, points_used 
            FROM payments 
            WHERE id = ? AND tenant_id = ?
        ");
        ...
        if (!$payment) {
            return ['success' => false, 'discount_applied' => 0, 'message' => 'Payment not found'];
        }
        
        $payment_amount = (float)$payment['amount'];
        $existing_discount = (float)($payment['discount_applied'] ?? 0);
        
        // Get all pending redemptions for this customer (ordered by oldest first)
        $stmt = $pdo->prepare("
            SELECT id, points_used, discount_amount
            FROM point_redemptions
            WHERE customer_id = ? AND tenant_id = ? AND status = 'pending'
            ORDER BY redemption_date ASC
        ");
        ...
        if (empty($redemptions)) {
            return ['success' => true, 'discount_applied' => 0, 'message' => 'No pending redemptions', 'redemptions' => []];
        }
        ...
        // First, show what redemptions are available
        $available_redemptions = [];
        foreach ($redemptions as $red) {
            $available_redemptions[] = [
                'id' => $red['id'],
                'discount' => (float)$red['discount_amount'],
                'points' => (float)$red['points_used']
            ];
        }
        
        // Apply redemptions until payment amount is used up or no more redemptions
        foreach ($redemptions as $redemption) {
            ...
            // Apply discount (but not more than remaining payment amount)
            $applied_discount = min($discount, $remaining_amount);
            $total_discount += $applied_discount;
            $remaining_amount -= $applied_discount;
            
            // Calculate points used proportionally
            $points_used_ratio = $applied_discount / $discount;
            $points_used = round((float)$redemption['points_used'] * $points_used_ratio, 2);
            
            $is_fully_applied = ($applied_discount >= $discount - 0.01);
            ...
            $applied_redemptions[] = [
                ...
                'fully_applied' => $is_fully_applied
            ];
        }
        
        if ($total_discount > 0) {
            // Calculate new payment amount
            $new_amount = max(0, $payment_amount - $total_discount);
            $total_points_used = $total_discount * 100; // Convert discount to points
            ...
            // Update each redemption status
            foreach ($applied_redemptions as $applied) {
                if ($applied['fully_applied']) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        WHERE id = ?
                    ");
                    $stmt->execute([$payment_id, $payment_id, $applied['id']]);
                } else {
                    // Partially applied - update the remaining amount
                    $remaining_discount = $applied['original_discount'] - $applied['applied_discount'];
                    $remaining_points = round($remaining_discount * 100, 2);
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        WHERE id = ?
                    ");
                    $stmt->execute([$remaining_discount, $remaining_points, $applied['id']]);
                    
                    // Log the applied portion
                    $stmt = $pdo->prepare("
                        INSERT INTO loyalty_points_log
                        ...
                        VALUES (?, ?, 0, ?, ?, 'Points redeemed - partial applied to payment #?', 'redemption_partial', ?, ?, NOW())
                    ");
                    $stmt->execute([
                        $tenant_id,
                        $customer_id,
                        $applied['points_used'],
                        $applied['applied_discount'],
                        $payment_id,
                        $applied['id'],
                        $customer_id
                    ]);
            ...
            $stmt = $pdo->prepare("
                INSERT INTO loyalty_points_log
                (tenant_id, customer_id, points_earned, points_redeemed, amount_earned, reason, reference_type, reference_id, created_by, created_at)
                VALUES (?, ?, 0, ?, ?, 'Discount applied from loyalty points', 'discount_applied', ?, ?, NOW())
            ");
            $stmt->execute([
                $tenant_id,
                $customer_id,
            ...
            return [
                ...
                'message' => '$' . number_format($total_discount, 2) . ' discount applied from loyalty points',
                'redemptions' => $available_redemptions,
                'applied_redemptions' => $applied_redemptions
            ];
        }
        ...
        error_log("Error applying redemption to payment: " . $e->getMessage());
        return ['success' => false, 'discount_applied' => 0, 'message' => $e->getMessage()];
    }
}

/**
 * Reverse redemption when payment is deleted
 */
function reverseRedemptionForPayment(PDO $pdo, int $payment_id, int $tenant_id): array
{
    try {
        // Find redemptions that were applied to this payment
        $stmt = $pdo->prepare("
            SELECT id, status, discount_amount, points_used, customer_id
            FROM point_redemptions
            WHERE applied_to_payment_id = ? AND tenant_id = ? AND status IN ('applied', 'partial')
        ");
        $stmt->execute([$payment_id, $tenant_id]);
        $redemptions = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        if (empty($redemptions)) {
            return ['success' => true, 'message' => 'No redemptions to reverse'];
        }
        ...
        foreach ($redemptions as $redemption) {
            ...
            ");
            $stmt->execute([
                (float)$redemption['points_used'],
                (int)$redemption['customer_id'],
                $tenant_id
            ]);
            ...
            $stmt = $pdo->prepare("
                ...
                WHERE id = ?
            ");
            $stmt->execute([$redemption['id']]);
            
            // Log the reversal
            $stmt = $pdo->prepare("
                INSERT INTO loyalty_points_log
                (tenant_id, customer_id, points_earned, points_redeemed, amount_earned, reason, reference_type, reference_id, created_by, created_at)
                VALUES (?, ?, ?, 0, 0, 'Points returned from deleted payment - redemption reversed', 'redemption_reversal', ?, ?, NOW())
            ");
            $stmt->execute([
                $tenant_id,
        ...
        return ['success' => true, 'message' => 'Redemptions reversed successfully'];
    } catch (Exception $e) {
        error_log("Error reversing redemption: " . $e->getMessage());
        return ['success' => false, 'message' => $e->getMessage()];
    }
}

/**
 * Generate a safe payment number for a tenant.
 */
function generatePaymentNumberForTenant(PDO $pdo, ?int $tenant_id = null): string
{
    $prefix = 'PMT';
    $year = date('Y');
    $month = date('m');
    ...
    if (!empty($tenant_id)) {
        // Create payment sequence automatically if missing
        $seqStmt = $pdo->prepare("
        ...
        // Move sequence forward only after number is prepared
    ...
    if ($last && !empty($last['payment_number'])) {
        ...
        $newSeq = ((int)end($parts)) + 1;
    }

    return $prefix . '-' . $year . $month . '-' . str_pad($newSeq, 5, '0', STR_PAD_LEFT);
}
...
if (isset($_GET['action'])) {
    ...
    if ($action === 'export_payments') {
        ...
        fputcsv($output, ['ID', 'Payment Number', 'Date', 'Entity', 'Invoice', 'Amount', 'Discount', 'Final Amount', 'Points Used', 'Category', 'Method', 'Reference', 'Tenant']);
        ...
        $sql = "SELECT p.*, t.name as tenant_name, c.customer_name, i.invoice_number 
                FROM payments p 
    ...
    if ($action === 'download_sample') {
        ...
        $output = fopen('php://output', 'w');
        fprintf($output, chr(0xEF).chr(0xBB).chr(0xBF));
        fputcsv($output, ['Tenant Name', 'Payment Number', 'Payment Date (YYYY-MM-DD)', 'Amount', 'Type (customer/supplier)', 'Customer/Supplier Name', 'Invoice Number', 'Method (cash/bank_transfer/check/mobile_money)', 'Reference', 'Category']);
        fputcsv($output, ['Example Logistics', 'PMT-001', date('Y-m-d'), '500.00', 'customer', 'John Doe', 'INV-5001', 'cash', '', 'Service Payment']);
        fclose($output);
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    // Ensure clean JSON output without PHP warnings breaking AJAX
    if (ob_get_length()) {
        ob_clean();
    }
    ...
    if ($action === 'get_payments') {
        ...
        <div style="overflow-x: auto; width: 100%;">
            <table class="payments-table" style="min-width: 1400px; width: 100%;">
                <thead>
                    <tr>
                        ...
                        <th style="padding: 12px 8px;">Qadarka Asalka</th>
                        <th style="padding: 12px 8px;">Dhimis</th>
                        <th style="padding: 12px 8px;">Qadarka Dambe</th>
                        <th style="padding: 12px 8px;">Points La Isticmaalay</th>
                        ...
                        <?php if ($is_superadmin): ?><th style="padding: 12px 8px;">Shirkadda</th><?php endif; ?>
                ...
                <tbody>
                    <?php if (count($payments) > 0): ?>
                        <?php foreach ($payments as $payment): 
                            ...
                            $original_amount = ($payment['amount'] + ($payment['discount_applied'] ?? 0));
                            $discount = $payment['discount_applied'] ?? 0;
                            ...
                            <tr>
                                ...
                                <td style="padding: 10px 8px;">
                                    ...
                                    <?php elseif ($payment['supplier_name'] && !$payment['customer_id']): ?>
                                        <div style="font-size: 10px; color: #e65100;">Alaab-qeybiye</div>
                                    <?php endif; ?>
                                 </td>
                                ...
                                <td style="padding: 10px 8px;"><span class="text-muted">$<?= number_format($original_amount, 2) ?></span> </td>
                                <td style="padding: 10px 8px;">
                                    <?php if ($discount > 0): ?>
                                        <span class="text-success">-$<?= number_format($discount, 2) ?></span>
                                        <div style="font-size: 10px;">(<?= number_format($points_used, 0) ?> pts)</div>
                                    <?php else: ?>
                                        <span class="text-muted">-</span>
                                    <?php endif; ?>
                                 </td>
                                <td style="padding: 10px 8px;"><strong class="text-danger">$<?= number_format($final_amount, 2) ?></strong> </td>
                                <td style="padding: 10px 8px;">
                                    <?php if ($points_used > 0): ?>
                                        <span class="badge badge-purple"><?= number_format($points_used, 0) ?></span>
                                    <?php else: ?>
                                        <span class="text-muted">-</span>
                                    <?php endif; ?>
                                 </td>
                                ...
                                <?php if ($is_superadmin): ?><td style="padding: 10px 8px;"><?= htmlspecialchars($payment['tenant_name'] ?? '-') ?> </td><?php endif; ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="<?= $is_superadmin ? 17 : 16 ?>" style="text-align: center; padding: 50px;">
    ...
    elseif ($action === 'get_payment') {
        ...
        try {
            if (!$is_superadmin) {
                $check = $pdo->prepare("SELECT p.id FROM payments p WHERE p.id = ? AND p.tenant_id = ?");
                $check->execute([$id, $user_tenant_id]);
                if (!$check->fetch()) {
    ...
    elseif ($action === 'get_customer_pending_redemptions') {
        $customer_id = isset($_POST['customer_id']) ? (int)$_POST['customer_id'] : 0;
        $tenant_id = isset($_POST['tenant_id']) ? (int)$_POST['tenant_id'] : 0;
        
        if (!$customer_id || !$tenant_id) {
            echo json_encode(['success' => false, 'redemptions' => [], 'total_discount' => 0]);
            exit;
        }
        
        $total_discount = getPendingRedemptionDiscount($pdo, $customer_id, $tenant_id);
        $redemptions = getPendingRedemptionsList($pdo, $customer_id, $tenant_id);
    ...
    elseif ($action === 'save_payment') {
        ...
        try {
            ...
            if ($bank_account_id && !$is_superadmin) {
            ...
            if (empty($id)) {
                ...
                if ($payment_type === 'customer') {
                    ...
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([$tenant_id, $payment_number, $customer_id, $invoice_id, $amount, $payment_date, $payment_method, $category, $reference_number, $bank_account_id, $notes, $_SESSION['user_id']]);
                } else {
                    ...
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([$tenant_id, $payment_number, $supplier_name, $amount, $payment_date, $payment_method, $category, $reference_number, $bank_account_id, $notes, $_SESSION['user_id']]);
                }
                
                $new_payment_id = $pdo->lastInsertId();
                
                if ($payment_type === 'customer' && $customer_id) {
                    // Check for pending redemptions BEFORE updating debt
                    $pending_discount = getPendingRedemptionDiscount($pdo, $customer_id, $tenant_id);
                    
                    if ($pending_discount > 0) {
                        // Apply redemption discount to this payment
                        $redemption_result = applyPendingRedemptionToPayment($pdo, (int)$new_payment_id, $customer_id, $tenant_id);
                        
                        if ($redemption_result['success'] && $redemption_result['discount_applied'] > 0) {
                            // Get the updated payment amount after discount
                            ...
                            $final_amount = (float)$updated_payment['amount'];
                            
                            // Update customer debt with the discounted amount
                            ...
                            // Add note about discount applied
                            $note_stmt = $pdo->prepare("UPDATE payments SET notes = CONCAT(COALESCE(notes, ''), '\n', 'Discount applied from loyalty points: $', ?) WHERE id = ?");
                            $note_stmt->execute([$redemption_result['discount_applied'], $new_payment_id]);
                        } else {
                            // No discount applied, update debt with full amount
                            $updateDebt = $pdo->prepare("UPDATE customers SET debt_amount = debt_amount - ?, updated_at = NOW() WHERE id = ? AND tenant_id = ?");
                            $updateDebt->execute([$amount, $customer_id, $tenant_id]);
                        }
                    } else {
                        // No pending redemptions, update debt with full amount
                        $updateDebt = $pdo->prepare("UPDATE customers SET debt_amount = debt_amount - ?, updated_at = NOW() WHERE id = ? AND tenant_id = ?");
                        $updateDebt->execute([$amount, $customer_id, $tenant_id]);
                    }
                    
                    // Award loyalty points for customer payment (based on original amount before discount)
                    awardLoyaltyPointsForPayment($pdo, (int)$new_payment_id, (int)($_SESSION['user_id'] ?? 0));
                }
                
                if ($payment_type === 'customer') {
                    $cashStmt = $pdo->prepare("INSERT INTO cash_flow (tenant_id, flow_date, inflow, description, created_at) VALUES (?, ?, ?, ?, NOW())");
                    $cashStmt->execute([$tenant_id, $payment_date, $amount, "Bixin macaamil: $payment_number"]);
                } else {
                    $cashStmt = $pdo->prepare("INSERT INTO cash_flow (tenant_id, flow_date, outflow, description, created_at) VALUES (?, ?, ?, ?, NOW())");
                    $cashStmt->execute([$tenant_id, $payment_date, $amount, "Bixin alaab-qeybiye: $payment_number"]);
                }
                
                // --- ERP INTEGRATION: POST TO LEDGER ---
                ...
                // Get final payment info for response
                $stmt = $pdo->prepare("SELECT amount, discount_applied FROM payments WHERE id = ?");
                $stmt->execute([$new_payment_id]);
                $final_payment = $stmt->fetch(PDO::FETCH_ASSOC);
                $discount_msg = '';
                if (($final_payment['discount_applied'] ?? 0) > 0) {
                    $discount_msg = "<br><span class='text-success'><i class='fas fa-gift'></i> Dhimis laga sameeyay loyalty points: $" . number_format($final_payment['discount_applied'], 2) . "</span>";
                }
                
                echo json_encode([
                    'success' => true, 
                    'message' => "Bixinta '$$amount' oo loogu talagalay $type_text waa la diiwaangeliyay!<br>Lambarka: <strong>$payment_number</strong>$discount_msg",
                    'payment_id' => $new_payment_id,
                    'payment_number' => $payment_number
                ]);
            } else {
                // Update existing payment
                $oldStmt = $pdo->prepare("SELECT customer_id, invoice_id, amount, payment_method, bank_account_id FROM payments WHERE id = ?");
                $oldStmt->execute([$id]);
                $oldPayment = $oldStmt->fetch();
                
                // Reverse old loyalty points before updating payment
                reverseLoyaltyPointsForPayment($pdo, (int)$id);
                
                // Reverse any redemptions applied to this payment
                reverseRedemptionForPayment($pdo, (int)$id, $tenant_id);

                if ($oldPayment && $oldPayment['customer_id']) {
                    $revertDebt = $pdo->prepare("UPDATE customers SET debt_amount = debt_amount + ? WHERE id = ?");
                    $revertDebt->execute([$oldPayment['amount'], $oldPayment['customer_id']]);
                    
                    if ($oldPayment['invoice_id']) {
                        $revertInv = $pdo->prepare("UPDATE invoices SET paid_amount = paid_amount - ?, status = CASE WHEN (total_amount - (paid_amount - ?)) <= 0 THEN 'paid' ELSE 'unpaid' END, updated_at = NOW() WHERE id = ?");
                        $revertInv->execute([$oldPayment['amount'], $oldPayment['amount'], $oldPayment['invoice_id']]);
                    }
                }
                
                if ($oldPayment && $oldPayment['payment_method'] === 'bank_transfer' && $oldPayment['bank_account_id']) {
                    $revertStmt = $pdo->prepare("UPDATE bank_accounts SET current_balance = current_balance + ? WHERE id = ?");
                    $revertStmt->execute([$oldPayment['amount'], $oldPayment['bank_account_id']]);
                }
                
                if ($payment_type === 'customer') {
                    $sql = "UPDATE payments SET tenant_id = ?, payment_number = ?, customer_id = ?, invoice_id = ?, amount = ?, payment_date = ?, payment_method = ?, category = ?, reference_number = ?, bank_account_id = ?, notes = ? WHERE id = ?";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([$tenant_id, $payment_number, $customer_id, $invoice_id, $amount, $payment_date, $payment_method, $category, $reference_number, $bank_account_id, $notes, $id]);
                    
                    if ($customer_id) {
                        $updateDebt = $pdo->prepare("UPDATE customers SET debt_amount = debt_amount - ? WHERE id = ?");
                        $updateDebt->execute([$amount, $customer_id]);
                    }
                    
                    if ($invoice_id) {
                        $updateInv = $pdo->prepare("UPDATE invoices SET paid_amount = paid_amount + ?, status = CASE WHEN (paid_amount + ?) >= total_amount THEN 'paid' ELSE 'partial' END, updated_at = NOW() WHERE id = ?");
                        $updateInv->execute([$amount, $amount, $invoice_id]);
                    }
                } else {
                    $sql = "UPDATE payments SET tenant_id = ?, payment_number = ?, supplier_name = ?, amount = ?, payment_date = ?, payment_method = ?, category = ?, reference_number = ?, bank_account_id = ?, notes = ? WHERE id = ?";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([$tenant_id, $payment_number, $supplier_name, $amount, $payment_date, $payment_method, $category, $reference_number, $bank_account_id, $notes, $id]);
                }
                ...
                if ($payment_type === 'customer' && $customer_id) {
                    ...
                    if ($pending_discount > 0) {
                        $redemption_result = applyPendingRedemptionToPayment($pdo, (int)$id, $customer_id, $tenant_id);
                    }
                    
                    // Recalculate loyalty points after payment update
                    awardLoyaltyPointsForPayment($pdo, (int)$id, (int)($_SESSION['user_id'] ?? 0));
                }
                
                if ($payment_method === 'bank_transfer' && $bank_account_id) {
                    $newStmt = $pdo->prepare("UPDATE bank_accounts SET current_balance = current_balance - ? WHERE id = ?");
                    $newStmt->execute([$amount, $bank_account_id]);
                }
    ...
    elseif ($action === 'delete_payment') {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT payment_number, amount, payment_method, bank_account_id, tenant_id, payment_date, customer_id, invoice_id, supplier_name FROM payments WHERE id = ?");
            $stmt->execute([$id]);
            $payment = $stmt->fetch();
            ...
            // Reverse loyalty points before deleting payment
            reverseLoyaltyPointsForPayment($pdo, (int)$id);
            
            // Reverse any redemptions applied to this payment
            reverseRedemptionForPayment($pdo, (int)$id, $payment['tenant_id']);

            if ($payment['customer_id']) {
                $revertDebt = $pdo->prepare("UPDATE customers SET debt_amount = debt_amount + ? WHERE id = ?");
                $revertDebt->execute([$payment['amount'], $payment['customer_id']]);
                
                if ($payment['invoice_id']) {
                    $revertInv = $pdo->prepare("UPDATE invoices SET paid_amount = paid_amount - ?, status = CASE WHEN (total_amount - (paid_amount - ?)) <= 0 THEN 'paid' ELSE 'unpaid' END, updated_at = NOW() WHERE id = ?");
                    $revertInv->execute([$payment['amount'], $payment['amount'], $payment['invoice_id']]);
                }
            }
            
            if ($payment['payment_method'] === 'bank_transfer' && $payment['bank_account_id']) {
                $stmt = $pdo->prepare("UPDATE bank_accounts SET current_balance = current_balance + ? WHERE id = ?");
                $stmt->execute([$payment['amount'], $payment['bank_account_id']]);
            }
            
            $stmt = $pdo->prepare("DELETE FROM payments WHERE id = ?");
            $stmt->execute([$id]);
            
            $cashStmt = $pdo->prepare("DELETE FROM cash_flow WHERE description LIKE ? AND (inflow = ? OR outflow = ?) AND flow_date = ?");
            $cashStmt->execute(["%{$payment['payment_number']}%", $payment['amount'], $payment['amount'], $payment['payment_date']]);
    ...
    elseif ($action === 'get_stats') {
        ...
        $stmt = $pdo->prepare("
            SELECT 
                ...
                SUM(amount + COALESCE(discount_applied, 0)) as total_original_amount,
                SUM(amount) as total_amount,
                SUM(COALESCE(discount_applied, 0)) as total_discount,
                SUM(COALESCE(points_used, 0)) as total_points_used,
    ...
    elseif ($action === 'generate_payment_number') {
        $tenant_id = !empty($_POST['tenant_id']) ? (int)$_POST['tenant_id'] : ($is_superadmin ? null : $user_tenant_id);
        ...
        try {
            $payment_number = generatePaymentNumberForTenant($pdo, $tenant_id);
    ...
    elseif ($action === 'get_customers_by_tenant') {
        ...
        if (!$tenant_id) {
            echo json_encode(['success' => false, 'customers' => [], 'empty' => true, 'message' => 'Fadlan dooro shirkad marka hore']);
            exit;
        }
        ...
        // For each customer, get pending redemptions
        foreach ($customers as &$customer) {
            $customer['pending_redemptions'] = getPendingRedemptionDiscount($pdo, $customer['id'], $tenant_id);
        }
        
        $empty = count($customers) === 0;
        $message = $empty ? "Ma jiraan macaamiilo ku xiran shirkaddan" : "";
    ...
    elseif ($action === 'get_invoices_by_customer') {
        ...
        if (!$customer_id) {
            echo json_encode(['success' => false, 'invoices' => [], 'empty' => true, 'message' => 'Fadlan dooro macaamilka marka hore']);
            exit;
        }
        ...
        $stmt = $pdo->prepare("
            ...
            FROM invoices i 
            WHERE i.customer_id = ? AND i.status != 'paid' AND (i.total_amount - i.paid_amount) > 0 
        ...
        $message = $empty ? "Ma jiraan biilalo aan la bixin oo ku xiran macaamilkan" : "";
    ...
    elseif ($action === 'import_payments') {
        ...
        try {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                $line++;
                // Columns: Tenant Name, Payment Number, Payment Date, Amount, Type, Customer/Supplier Name, Invoice Number, Method, Reference, Category
                $tenant_name = trim($data[0] ?? '');
                $payment_number = trim($data[1] ?? '');
                $payment_date = trim($data[2] ?? date('Y-m-d'));
                $amount = (float)(str_replace(['$', ','], '', $data[3] ?? 0));
                $type = strtolower(trim($data[4] ?? 'customer'));
                $entity_name = trim($data[5] ?? '');
                $invoice_number = trim($data[6] ?? '');
                $method = strtolower(trim($data[7] ?? 'cash'));
                $reference = trim($data[8] ?? '');
                $category = trim($data[9] ?? '');
                
                if (empty($tenant_name) || empty($payment_number) || empty($entity_name)) continue;
                ...
                $stmt->execute([$t_id, $payment_number]);
                if ($stmt->fetch()) {
                    $errors[] = "Line $line: Payment #$payment_number already exists for tenant '$tenant_name'.";
                    continue;
                }
                ...
                $stmt = $pdo->prepare("INSERT INTO payments (tenant_id, payment_number, customer_id, supplier_name, invoice_id, amount, payment_date, payment_method, category, reference_number, created_by, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,NOW())");
                $stmt->execute([$t_id, $payment_number, $customer_id, $supplier_name, $invoice_id, $amount, $payment_date, $method, $category, $reference, $user_id]);
                $new_pay_id = $pdo->lastInsertId();
                
                if ($customer_id) {
                    // Check for pending redemptions
                    $pending_discount = getPendingRedemptionDiscount($pdo, $customer_id, $t_id);
                    
                    if ($pending_discount > 0) {
                        $redemption_result = applyPendingRedemptionToPayment($pdo, (int)$new_pay_id, $customer_id, $t_id);
                        
                        if ($redemption_result['success'] && $redemption_result['discount_applied'] > 0) {
                            $stmt = $pdo->prepare("SELECT amount FROM payments WHERE id = ?");
                            $stmt->execute([$new_pay_id]);
                            $updated_payment = $stmt->fetch(PDO::FETCH_ASSOC);
                            $final_amount = (float)$updated_payment['amount'];
                            
                            $stmt = $pdo->prepare("UPDATE customers SET debt_amount = debt_amount - ? WHERE id = ?");
                            $stmt->execute([$final_amount, $customer_id]);
                        } else {
                            $stmt = $pdo->prepare("UPDATE customers SET debt_amount = debt_amount - ? WHERE id = ?");
                    ...
                    // Automatic loyalty points for imported customer payment
                    awardLoyaltyPointsForPayment($pdo, (int)$new_pay_id, (int)$user_id);
                }
...
<head>
    ...
    <title>Maareynta Bixinta - <?= $is_superadmin ? 'Super Admin' : htmlspecialchars($user_tenant['tenant_name'] ?? '') ?> | Cargo Management System</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-purple { background: #f3e5f5; color: #7b1fa2; padding: 4px 8px; border-radius: 12px; font-size: 11px; }
        ...
        .alert-info { background: #e3f2fd; color: #0077c5; border-left: 4px solid #0077c5; }
        ...
        .pending-redemption-badge { background: #fff3e0; color: #e65100; padding: 4px 8px; border-radius: 12px; font-size: 11px; margin-left: 8px; }
        ...
        .receipt-preview .receipt-header {
            background: linear-gradient(135deg, #2D1859 0%, #4B2C85 100%);
            color: white;
            padding: 20px;
            text-align: center;
            border-radius: 8px 8px 0 0;
        }
        .receipt-preview .amount-number {
            font-size: 36px;
            font-weight: bold;
            color: #0F7A3A;
        }
...
<div class="container-fluid" style="padding: 20px;">
    ...
    <div class="page-header">
        ...
        <div class="d-flex align-items-center">
            <?php if (!$is_superadmin && $user_tenant): ?>
            ...
            <div class="dropdown ml-2">
                ...
                <div class="dropdown-menu dropdown-menu-right" style="border-radius: 8px; box-shadow: 0 5px 15px rgba(0,0,0,0.1);">
                    <a class="dropdown-item" href="?action=export_payments" id="exportPaymentsBtn"><i class="fas fa-download mr-2"></i> Export Payments</a>
    ...
    <div class="stats-grid" id="stats-container">
        ...
        <div class="stat-card"><div class="stat-info"><h4>Wadarta Dhimista</h4><div class="stat-number" id="stat-total-discount">$0</div></div><div class="stat-icon"><i class="fas fa-tag"></i></div></div>
        ...
        <div class="stat-card"><div class="stat-info"><h4>Lacagta Caddaanka ah</h4><div class="stat-number" id="stat-cash">$0</div></div><div class="stat-icon"><i class="fas fa-money-bill"></i></div></div>
    </div>
...
<div class="modal fade" id="paymentModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="paymentModalLabel"><i class="fas fa-money-bill-wave"></i> Bixin Cusub</h5>
            ...
            <form id="paymentForm">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <?php else: ?>
                        <input type="hidden" name="tenant_id" id="modalTenantId" value="<?= $user_tenant_id ?>">
                        <?php endif; ?>
                    </div>
                    ...
                    <div id="customerPaymentSection">
                        ...
                        <div id="redemptionNotice" class="redemption-notice" style="display: none;">
                            <i class="fas fa-gift"></i> <strong>Pending Redemptions!</strong><br>
                            Macaamilkan wuxuu leeyahay <span id="redemptionCount">0</span> dhimis sugaya wadarta <strong id="redemptionTotal">$0.00</strong><br>
                            <small>Markaad bixinta kaydiso, dhimistaa si otomatik ah ayaa lagugu dabaqi doonaa.</small>
                        </div>
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="category" id="modalCategory" class="form-control">
                                    ...
                                    <option value="Rental">Rental (Kiro)</option>
                                    <option value="Fuel">Fuel (Shidaal)</option>
                                    <option value="Maintenance">Maintenance</option>
                                    <option value="Salary">Salary</option>
                                    <option value="Supplier">Supplier</option>
                                    <option value="Customer Payment">Customer Payment</option>
...
<div class="modal fade" id="viewModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-money-bill-wave"></i> Faahfaahinta Bixinta</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
...
$(document).ready(function() {
    ...
    let isSuperadmin = <?= json_encode($is_superadmin) ?>;
    let currentReceiptPaymentId = null;

    // Function to open receipt popup
            ...
            error: function(xhr, status, error) {
                ...
                $('#receiptModalBody').html(`
                    <div class="alert alert-danger text-center">
                        ...
                        <h5 class="mt-3">Khalad ayaa dhacay</h5>
                        <p>Lamaan soo dejinta rasiidka. Fadlan isku day mar kale.</p>
    ...
    $('#printReceiptBtn').click(function() {
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>Rasiidka Bixinta - ${currentReceiptPaymentId}</title>
                <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
                <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    .receipt-header { background: linear-gradient(135deg, #2D1859 0%, #4B2C85 100%); color: white; padding: 20px; text-align: center; }
    ...
    function generatePaymentNumber(tenantId) {
        return new Promise(function(resolve, reject) {
            if (!tenantId) {
                $('#autoPaymentNumber').text('Dooro shirkad');
                $('#modalPaymentNumber').val('');
                reject('Fadlan dooro shirkad');
                return;
            }

            $('#autoPaymentNumber').html('<i class="fas fa-spinner fa-spin"></i> Generating...');

            $.ajax({
                url: window.location.href,
                type: 'POST',
                data: { ajax_action: 'generate_payment_number', tenant_id: tenantId },
                dataType: 'json',
                success: function(res) {
                    if (res.success && res.payment_number) {
                        ...
                        $('#modalPaymentNumber').val('');
                        reject(res.message || 'Payment number lama sameyn karo');
                    }
                },
                error: function(xhr) {
                    console.error('Error generating payment number:', xhr.responseText || xhr);
                    $('#autoPaymentNumber').text('Error');
                    $('#modalPaymentNumber').val('');
    ...
    function loadPendingRedemptions(customerId, tenantId) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { 
                ajax_action: 'get_customer_pending_redemptions', 
                customer_id: customerId, 
                tenant_id: tenantId 
            },
            dataType: 'json',
            success: function(res) {
                if (res.success && res.total_discount > 0) {
                    $('#redemptionCount').text(res.count);
                    $('#redemptionTotal').text('$' + parseFloat(res.total_discount).toFixed(2));
                    $('#redemptionNotice').show();
                } else {
                    $('#redemptionNotice').hide();
                }
            },
    ...
    function loadCustomersByTenant(tenantId, selectedCustomerId = null, done = null) {
        const select = $('#modalCustomerId');
        if (!tenantId) {
            select.html('<option value="">-- Dooro Shirkad --</option>');
            if (typeof done === 'function') done(false);
            return;
        }

        $('#customerLoading').show();
        select.prop('disabled', true).html('<option value="">Loading customers...</option>');

        $.ajax({
            url: window.location.href,
            type: 'POST',
            data: { ajax_action: 'get_customers_by_tenant', tenant_id: tenantId },
            dataType: 'json',
            success: function(res) {
                select.empty();
                ...
                if (res.success && res.customers && res.customers.length > 0) {
                    ...
                    $.each(res.customers, function(i, c) {
                        ...
                        if (c.pending_redemptions > 0) {
                            pendingBadge = ' <span class="pending-redemption-badge"><i class="fas fa-gift"></i> ' + c.pending_redemptions + '</span>';
                        }
                        select.append('<option value="' + c.id + '" data-debt="' + (c.debt_amount || 0) + '" data-pending="' + (c.pending_redemptions || 0) + '">' + escapeHtml(c.customer_name) + ' - Deynta: $' + parseFloat(c.debt_amount || 0).toFixed(2) + pendingBadge + '</option>');
                    });

                    if (selectedCustomerId) {
                        select.val(String(selectedCustomerId));
                        if (!select.val()) {
                            select.append('<option value="' + selectedCustomerId + '" selected>Customer ID #' + selectedCustomerId + '</option>');
                        }
                    }
                } else {
            ...
            error: function(xhr) {
                ...
                select.html('<option value="">Error loading customers</option>').prop('disabled', false);
                $('#customerLoading').hide();
                if (typeof done === 'function') done(false);
            }
        });
    ...
    function loadInvoicesByCustomer(customerId, selectedInvoiceId = null, done = null) {
        ...
        $('#invoiceLoading').show();
        select.prop('disabled', true).html('<option value="">Loading invoices...</option>');
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (selectedInvoiceId) {
                    select.val(String(selectedInvoiceId));
                    if (!select.val()) {
                        select.append('<option value="' + selectedInvoiceId + '" selected>Invoice ID #' + selectedInvoiceId + '</option>');
                    }
                }
            ...
            error: function(xhr) {
                ...
                select.html('<option value="">Error loading invoices</option>').prop('disabled', false);
                $('#invoiceLoading').hide();
                if (typeof done === 'function') done(false);
            }
        });
    ...
    $('#modalInvoiceId').on('change', function() {
        ...
        if (invoiceId) {
            ...
            $('#amountHint').html('<span class="text-success"><i class="fas fa-check-circle"></i> Qadarka bixinta ayaa si otomatik ah loo qaaday ($' + parseFloat(dueAmount).toFixed(2) + ')</span>');
    ...
    $('#modalCustomerId').on('change', function() {
        ...
        const tenantId = $('#modalTenantId').val();
        ...
        if (customerId) {
            ...
            if (tenantId) {
                loadPendingRedemptions(customerId, tenantId);
            }
        } else {
    ...
    if (isSuperadmin) {
        $('#modalTenantId').on('change', function() {
            ...
            if (tenantId) {
                generatePaymentNumber(tenantId);
                loadCustomersByTenant(tenantId);
            } else {
                $('#autoPaymentNumber').text('Dooro shirkad');
                $('#modalCustomerId').html('<option value="">-- Dooro Shirkad --</option>');
            }
        });
    } else {
        const tenantId = <?= json_encode($user_tenant_id) ?>;
        if (tenantId) {
            generatePaymentNumber(tenantId);
            loadCustomersByTenant(tenantId);
        }
    ...
    function loadPayments() {
        ...
        if (isSuperadmin) data.tenant = $('#tenantFilter').val();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    let tenant = isSuperadmin ? $('#tenantFilter').val() : '';
                    $('#exportPaymentsBtn').attr('href', `?action=export_payments&search=${encodeURIComponent(search)}&tenant=${tenant}`);
                } else {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    $('#stat-total-discount').text('$' + (parseFloat(stats.total_discount || 0).toFixed(2)));
                    ...
                    monthlyChart = new Chart(document.getElementById('monthlyChart'), {
                        ...
                        options: {
                            ...
                            scales: { y: { beginAtZero: true, title: { display: true, text: 'Lacag ($)' } } }
                        }
                    });
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.success && res.data) {
                        const p = res.data;
                        const originalAmount = parseFloat(p.amount) + (parseFloat(p.discount_applied) || 0);
                        $('#viewModalBody').html(`
                            <div class="row">
                                <div class="col-12 mb-3">
                                    <div class="alert alert-info">
                                        <strong><i class="fas fa-receipt"></i> Lambarka Bixinta:</strong> ${escapeHtml(p.payment_number)}
                                    </div>
                                </div>
                                <div class="col-6"><strong>Taariikhda:</strong></div>
                                <div class="col-6">${p.payment_date}</div>
                                <div class="col-6"><strong>Macaamilka / Alaab-qeybiyaha:</strong></div>
                                <div class="col-6">${escapeHtml(p.customer_name || p.supplier_name || '-')}</div>
                                <div class="col-6"><strong>Qadarka Asalka:</strong></div>
                                <div class="col-6">$${originalAmount.toFixed(2)}</div>
                                ${p.discount_applied > 0 ? `
                                <div class="col-6"><strong>Dhimis (Loyalty):</strong></div>
                                <div class="col-6"><span class="text-success">-$${parseFloat(p.discount_applied).toFixed(2)}</span></div>
                                <div class="col-6"><strong>Points La Isticmaalay:</strong></div>
                                <div class="col-6">${parseFloat(p.points_used || 0).toFixed(0)} pts</div>
                                ` : ''}
                                <div class="col-6"><strong>Qadarka Dambe:</strong></div>
                                <div class="col-6"><strong class="text-danger">$${parseFloat(p.amount).toFixed(2)}</strong></div>
                                <div class="col-6"><strong>Habka Bixinta:</strong></div>
                                <div class="col-6">${p.payment_method}</div>
                                <div class="col-6"><strong>Lambarka Tixraaca:</strong></div>
                                <div class="col-6">${escapeHtml(p.reference_number || '-')}</div>
                                <div class="col-6"><strong>Nooca Bixinta:</strong></div>
                                <div class="col-6">${escapeHtml(p.category || '-')}</div>
                                <div class="col-12 mt-3"><strong>Qoraal:</strong></div>
                ...
                success: function(res) {
                    if (res.success && res.data) {
                        ...
                        if (p.customer_id) {
                            ...
                            loadCustomersByTenant(p.tenant_id, p.customer_id, function() {
    ...
    function showAlert(type, msg) {
        $('#alert-placeholder').html(`
            <div class="alert alert-${type} alert-dismissible fade show">
                <i class="fas ${type === 'success' ? 'fa-check-circle' : type === 'warning' ? 'fa-exclamation-triangle' : 'fa-exclamation-circle'}"></i> ${msg}
    ...
    $('#paymentForm').submit(async function(e) {
        ...
        if (isNaN(amount) || amount <= 0) {
            showAlert('error', 'Fadlan geli qadarka bixinta');
            return;
        }
        ...
        if (paymentType === 'customer') {
            ...
            if (!customerId) {
                showAlert('error', 'Fadlan dooro macaamilka');
                return;
            }
            ...
            if (invoiceId) {
                ...
                if (dueAmount > 0 && amount > dueAmount) {
                    showAlert('error', 'Qadarka bixinta ($' + amount.toFixed(2) + ') wuu ka badan yahay deynta biilka ($' + dueAmount.toFixed(2) + ')');
                    return;
                }
            }
        } else {
            ...
            if (!supplierName) {
                showAlert('error', 'Fadlan geli magaca alaab-qeybiyaha');
                return;
            }
        }
        ...
        if (method !== 'cash' && referenceNumber === '') {
            showAlert('error', 'Lambarka tixraaca waa lagama maarmaan');
            return;
        }
        if (method === 'bank_transfer' && !$('#modalBankAccountId').val()) {
            showAlert('error', 'Fadlan dooro xisaabta bangiga');
            return;
        }
        ...
        try {
            if (!$('#modalPaymentNumber').val()) {
                await generatePaymentNumber($('#modalTenantId').val());
            }
        } catch (err) {
            showAlert('error', err || 'Payment number lama sameyn karo');
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    ...
                    $('#amountHint').html('');
                    if (isSuperadmin) {
                        generatePaymentNumber($('#modalTenantId').val());
                    } else {
                        generatePaymentNumber(<?= json_encode($user_tenant_id) ?>);
                    }
                    toggleReferenceField();
                    togglePaymentType('customer');
            ...
            error: function(xhr) {
                let errorMsg = 'Khalad ayaa dhacay';
                try {
    ...
    $('#addPaymentBtn, #addPaymentBtnEmpty').click(function() {
        ...
        if (isSuperadmin) {
            ...
            if (tenantId) {
                ...
                $('#autoPaymentNumber').text('Fadlan dooro shirkad');
                $('#modalCustomerId').html('<option value="">-- Dooro Shirkad --</option>');
            }
        } else {
            generatePaymentNumber(<?= json_encode($user_tenant_id) ?>);
            loadCustomersByTenant(<?= json_encode($user_tenant_id) ?>);
        }
        toggleReferenceField();
        $('#paymentModal').modal('show');
    });
    ...
    $('#resetFilters').click(function() {
        $('#searchInput').val('');
        if (isSuperadmin) $('#tenantFilter').val('0');
